// OLED animation frames stored in PROGMEM.
// Sections are enabled via OLED_ANIMS_GHOST and/or OLED_ANIMS_FISHING
// before including this header.

#ifdef OLED_ANIMS_GHOST
// Ghost-related OLED animations (frames stored in PROGMEM)
// This file is intended to be included inside the
// master_render_ghost() function so arrays remain function-local.

// static const char PROGMEM hide[2][ANIM_SIZE_GHOST] = {
//     { // 'hide1' 32x28px
//         0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x40, 0x40, 0x40, 0x20, 0x20,
//         0x20, 0x20, 0x20, 0x20, 0x40, 0x40, 0x80, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
//         0x00, 0x00, 0x00, 0x00, 0x60, 0x90, 0x08, 0x04, 0x02, 0x01, 0x00, 0x40, 0x20, 0x10, 0x08, 0x08,
//         0x08, 0x08, 0x88, 0x70, 0x00, 0x70, 0x88, 0x08, 0x09, 0x12, 0x04, 0x18, 0xe0, 0x00, 0x00, 0x00,
//         0x00, 0x00, 0x00, 0x00, 0x66, 0x99, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x40,
//         0x22, 0x11, 0x08, 0x08, 0x08, 0x08, 0x10, 0x21, 0x02, 0x00, 0x00, 0xc0, 0x3f, 0x00, 0x00, 0x00,
//         0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x10, 0x20, 0x20, 0x20, 0x40,
//         0x40, 0x40, 0x40, 0x40, 0x20, 0x20, 0x10, 0x10, 0x08, 0x04, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00
//     },
//     { // 'hide2' 32x28px
//         0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x40, 0x40, 0x40, 0x20, 0x20,
//         0x20, 0x20, 0x20, 0x20, 0x40, 0x40, 0x80, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
//         0x00, 0x00, 0x00, 0x00, 0x60, 0x90, 0x08, 0x04, 0x02, 0x01, 0x00, 0x40, 0x20, 0x10, 0x08, 0x08,
//         0x08, 0x08, 0x88, 0x70, 0x00, 0x70, 0x88, 0x08, 0x09, 0x12, 0x04, 0x18, 0xe0, 0x00, 0x00, 0x00,
//         0x00, 0x00, 0x00, 0x00, 0x66, 0x99, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80,
//         0x62, 0x31, 0x38, 0x38, 0x38, 0x38, 0x30, 0x61, 0x82, 0x00, 0x00, 0xc0, 0x3f, 0x00, 0x00, 0x00,
//         0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x10, 0x20, 0x20, 0x20, 0x40,
//         0x40, 0x40, 0x40, 0x40, 0x20, 0x20, 0x10, 0x10, 0x08, 0x04, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00
//     }
// };

static const char PROGMEM shyguy[2][ANIM_SIZE_GHOST] = {
    { // 'shyguy1' 32x28px
        
    },
    { // 'shyguy2' 32x28px
        
    }
};

static const char PROGMEM eerie[2][ANIM_SIZE_GHOST] = {
    { // 'eerie1' 32x28px
        
    },
    { // 'eerie2' 32x28px
        
    }
};

static const char PROGMEM scare[2][ANIM_SIZE_GHOST] = {
    { // 'scare1' 32x28px
        
    },
    { // 'scare2' 32x28px
        
    }
};

static const char PROGMEM troll[2][ANIM_SIZE_GHOST] = {
    { // 'troll1' 32x28px
        
    },
    { // 'troll2' 32x28px
        
    }
};

static const char PROGMEM laugh[2][ANIM_SIZE_GHOST] = {
    { // 'laugh1' 32x28px
        
    },
    { // 'laugh2' 32x28px
        
    }
};
#endif

#ifdef OLED_ANIMS_FISHING
// Fishing animation frames for the slave OLED (stored in PROGMEM)
// Include this header inside the slave_render_ghost() function so the
// fishing[] symbol has local scope.

static const char PROGMEM fishing[2][416] = {
    { // 'fishing1' 32x100px
        
    },
    { // 'fishing2' 32x100px
        
    }
};
#endif
